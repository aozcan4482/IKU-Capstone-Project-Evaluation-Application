...
class _EvaluationScreenState extends State<EvaluationScreen> {
  ...
  Future<void> _initialize() async {
    try {
      ...
      final loaded = await EvaluationStorage.loadForProject(
  ...
  /// Draft kaydet (sadece SharedPreferences'e, backend'e gitmez)
  Future<void> _saveDraft(ProjectMember member, Map<String, double?> scores) async {
    final hasAny = scores.values.any((v) => v != null);
    final newEval = MemberEvaluation(
      scores: scores,
      status: hasAny ? MemberEvaluationStatus.inProgress : MemberEvaluationStatus.notStarted,
    );
  ...
  /// Final submit: backend'e POST + SharedPreferences'te status=submitted
  /// Dönen evaluation_id'leri topla (unlock request için lazım)
  Future<bool> _submit(ProjectMember member, Map<String, double?> scores) async {
    ...
    try {
      ...
      for (final c in _criteria) {
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300) {
          try {
            ...
            // parse hatası olursa id toplanmaz, submit yine başarılı sayılır
          }
        }
      }

      // Submitted olarak işaretle (evaluation_ids ile birlikte)
      final submittedEval = MemberEvaluation(
        scores: scores,
      ...
      setState(() {
        _evaluations[member.studentId] = submittedEval;
      });
  ...
  Widget _buildMemberCard(ProjectMember member) {
    ...
    final eval = _evaluations[member.studentId] ??
        MemberEvaluation.empty(_criteriaNames);
  ...
  Widget _buildLockedView(ProjectMember member, MemberEvaluation eval) {
    // Bu üye için unlock request atılabilir mi? Sadece backend id'leri varsa.
    // Eski (Adım A öncesi) submit edilmiş kayıtlarda evaluationIds boş olur.
                    ...
                    child: OutlinedButton.icon(
                      onPressed: isSaving
                          ...
                          : () async {
                              ...
                              await _saveDraft(member, scores);
                              if (!mounted) return;
                              setState(() {
                                _gradingMemberId = null;
                              });
                          ...
                          : () async {
                              ...
                              if (!allFilled) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                        'All criteria must be filled before submission.'),
                                    backgroundColor: ikuRed,
                                  ),
                                );
                                return;
  ...
  /// Final review bottom sheet: puanları özet göster + preview skor + Submit
  Future<void> _openReviewSheet(
      ProjectMember member, Map<String, double?> scores) async {
  ...
  /// Unlock request bottom sheet — jüri neden açılsın istiyor?
  Future<void> _openUnlockRequestSheet(
      ProjectMember member, MemberEvaluation eval) async {
                              ...
                              : () async {
                                  ...
                                  if (reason.isEmpty) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                            'Please provide a reason for the unlock request.'),
                                        backgroundColor: ikuRed,
                                      ),
                                    );
                                    return;
  ...
  /// Üyedeki tüm evaluation_id'ler için tek tek unlock request POST et
  /// Backend tarafında her biri ayrı bir UNLOCK_REQUESTS kaydı oluşturur
